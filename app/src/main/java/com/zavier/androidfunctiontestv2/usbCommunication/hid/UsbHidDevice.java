...
public class UsbHidDevice {
    ...
    public static UsbHidDevice factory(Context context, int vid, int pid) {
        try {
            ...
            if (devices.length == 0) {
                LogUtils.LogD(TAG,"devices.length = 0");
                return null;
            }
            return devices[0];
        } catch (Exception e) {
    ...
    public void write(byte[] data, int offset, int size) {
        ...
        if (mOutUsbEndpoint == null) {
            ...
            LogUtils.LogD(TAG,"endpoint is null");
        } else {
    ...
    public byte[] read(int size, int timeout) {
        ...
        LogUtils.LogD(TAG,"bytesRead: "+bytesRead);
        if (bytesRead <= size) {
    ...
    public int read(byte[] buf, int size, int timeout) {
        int bytesRead = mConnection.bulkTransfer(mInUsbEndpoint, buf, size, timeout);
        return bytesRead;
    }

    public String read(int size, int timeout, int flasg) {
        byte[] buffer = new byte[size];
        int bytesRead = mConnection.bulkTransfer(mInUsbEndpoint, buffer, size, timeout);
